...
export default defineComponent({
  ...
  setup() {
    ...
    const myPkHex = computed(() => keys.pkHex);
    ...
    function getMyGroupsSimple(m: any) {
      if (!msgs.inbox || !msgs.inbox.length) return null;

      const data = msgs.inbox.find(i => i.id === m.id);
      if (!data?.myGroups?.length) return null;

      return data.myGroups.map((g: string) => {
        const count = friends.list.filter(f =>
           (f.groups || f.group ? [f.group].flat() : ["未分组"]).includes(g)
         ).length;
