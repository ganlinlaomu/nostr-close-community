  ...
  notificationJumpDone.value = true;
        ...
        const stats = await backfillEvents({
          ...
          onComplete: () => {
            if (newestTimestamp > 0) {
             ...
             logger.info(
               `保存 backfill breakpoint = ${new Date(newestTimestamp * 1000).toLocaleString()}`
            );
          } else {
            logger.info("本次 backfill 没有新消息，不推进 breakpoint");
          }
        },
    ...
    async function startRealtimeSubscription(friendSet: Set<string>, relays: string[]) {
      try {
        ...
        // ⭐ 从本地取回填断点（关键）
        const rawBreakpoint =
          loadBackfillBreakpoint(`messages_${keys.pkHex}`);

        const messageBreakpoint =
          typeof rawBreakpoint === "number" && rawBreakpoint > 0
          ? rawBreakpoint
          : 0;
