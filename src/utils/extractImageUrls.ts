const mdImageRE = /!\[[^\]]*?\]\(\s*(https?:\/\/[^\s)]+)\s*\)/gi;
const plainImgUrlRE = /(https?:\/\/[^\s)]+?\.(?:png|jpe?g|gif|webp|avif|svg)(?:\?[^\s)]*)?)/gi;
// Match encrypted image references in markdown: ![](blossom+aesgcm:...)
const mdEncryptedImageRE = /!\[[^\]]*?\]\(\s*(blossom\+aesgcm:[^\s)]+)\s*\)/gi;

export function extractImageUrls(content: string): string[] {
  if (!content) return [];
  const urls: string[] = [];
  const seen = new Set<string>();
  let m: RegExpExecArray | null;
  
  // Extract markdown images (http(s) URLs)
  mdImageRE.lastIndex = 0;
  while ((m = mdImageRE.exec(content))) {
    const u = m[1].trim();
    if (!seen.has(u)) { seen.add(u); urls.push(u); }
  }
  
  // Extract encrypted image references from markdown
  mdEncryptedImageRE.lastIndex = 0;
  while ((m = mdEncryptedImageRE.exec(content))) {
    const u = m[1].trim();
    if (!seen.has(u)) { seen.add(u); urls.push(u); }
  }
  
  // Extract plain image URLs
  plainImgUrlRE.lastIndex = 0;
  while ((m = plainImgUrlRE.exec(content))) {
    const u = m[1].trim();
    if (!seen.has(u)) { seen.add(u); urls.push(u); }
  }
  
  return urls;
}
